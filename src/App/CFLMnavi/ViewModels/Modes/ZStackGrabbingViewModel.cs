...
public partial class ZStackGrabbingViewModel : ObservableObject, IDisposable
{
    ...
    private async Task ZStackAcquisitionImplementationAsync(IProgress<(string, Ratio)> progress, CancellationToken cancellationToken)
    {
        ...
        ZStackSettings zStackSettings = GetZStackSettings();
        ...
        if (lightSettings is [])
        {
            await _zStackGrabbingService.GrabZStackAsync(activeProject, _stageNavigation, _getCameraView.Invoke(), _pipelineGrabber, zStackSettings, progress, null, cancellationToken);
            return;
        }
    ...
    public ZStackSettings GetZStackSettings()
    {
        int numberOfSteps;
        Length stepSize;

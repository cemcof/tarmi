...
public partial class RoiImagesStateManager : ObservableObject
{
    ...
    private void ClearSecondaryImage()
    {
        ...
        {
            device.SecondaryFiducialPointControl.SetFiducialsSource(null, null);
    ...
    private async Task AddPrimaryImage(ImageChildVM childVM)
    {
        if (_virtualDeviceViewModel is VirtualDeviceViewModel device)
        {
            ...
            device.PrimaryFiducialPointControl.SetFiducialsSource(childVM, null);
    ...
    private static IEnumerable<ImageChildVM> GetAllSelectedImageChildren(IEnumerable<RoiChildVM>? nodes)
    {
        ...
        {
            if (node is ImageChildVM imageChild)
            {
                yield return imageChild;
            }
            else if (node is VirtualChildVM virtualChild)
            {
    ...
    private bool CanToggleImage(ImageChildVM childVM)
    {
        ...
        var correlationFyFiducials = _selectedRoi?.CorrelationByFiducials ?? false;
        if (correlationFyFiducials && childVM.ValidFiducialsCount < 3)
        {
    ...
    private void UpdateControllableZStack()
    {
        ...
        else if (secondaryStackVM is not null)
        {
    ...
    {
        if (_virtualDeviceViewModel is not null)
        {
    ...
    {
        _primaryTileSet.OnNext(_primarySource?.ParentVM is TileSetVirtualChildVM primaryTileSet ? primaryTileSet : null);
    }
        ...
        if (_virtualDeviceViewModel is VirtualDeviceViewModel device)
        {
            ...
            if (filePath?.IsNotNullOrEmpty() ?? false)
            {
                ...
                if (image is not null)
                {
                    ...
                    device.SecondaryFiducialPointControl.SetFiducialsSource(childVM, _primarySource);
    ...
    public async Task ClearState()
    {
        if (_virtualDeviceViewModel is VirtualDeviceViewModel device)
        {
            ...
            device.PrimaryFiducialPointControl.SetFiducialsSource(null, null);
        }
        _secondarySource?.Unselect();
        _secondarySource = null;

...
public static class ImageMetadataExtensions
{
    ...
    {
        var size = metadata.GetPixelSize();
        return new()
        {
            Height = size.Y * metadata.Coordinates!.ImageSize.Height,
            Width = size.X * metadata.Coordinates!.ImageSize.Width,
        };
    }
    ...
    public static LengthRectangle GetImageArea(this ImageMetadata metadata, Func<StagePosition, StageCameraView, LengthPoint> transformStageToPlanePosition)
    {
        ...
        var xFieldSizeHalf = pixelSize.X * imageSize.Width / 2;
        var yFieldSizeHalf = pixelSize.Y * imageSize.Height / 2;

        return new()
        {
            Bottom = planePosition.Y + yFieldSizeHalf,
            Left = planePosition.X - xFieldSizeHalf,
            Right = planePosition.X + xFieldSizeHalf,
            Top = planePosition.Y - yFieldSizeHalf
        };
    }

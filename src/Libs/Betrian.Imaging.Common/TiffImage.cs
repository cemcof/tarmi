...
public static class TiffImage
{
    ...
    public static ImageWithMetadata Load(string filePath)
    {
        ...
        try
        {
            ...
            if (matType == MatType.CV_8UC1) { image = new Image<Gray, byte>(mat); }
            else if (matType == MatType.CV_8UC3) { image = new Image<Bgr, byte>(mat); }
            else if (matType == MatType.CV_16UC1) { image = Image<Gray, byte>.ConvertFromMat(mat); }
            else if (matType == MatType.CV_16UC3) { image = Image<Bgr, byte>.ConvertFromMat(mat); }
            else { throw new NotSupportedException($"Unsupported image type: {mat.Type}"); }
